...
$('<li class="token" />').attr('data-value',value).append('<span>'+text+'</span>').prepend($('<a class="dismiss" />').on('mousedown touchstart',{},$.proxy(function(e){e.preventDefault();this.trigger('tokenize:tokens:remove',[value])},this))).insertBefore(this.searchContainer);this.trigger('tokenize:dropdown:hide');return this};Tokenize2.prototype.tokenRemove=function(v){var $item=$('option[value="'+v+'"]',this.element);if($item.attr('data-type')==='custom'){$item.remove()}else{$item.removeAttr('selected').prop('selected',!1)}
$('li.token[data-value="'+v+'"]',this.tokensContainer).remove();this.trigger('tokenize:tokens:reorder');return this};Tokenize2.prototype.remap=function(){var $selected=$('option:selected',this.element);$selected.each($.proxy(function(v,t){this.trigger('tokenize:tokens:add',[$(t).val(),$(t).html(),!1])},this));return this};Tokenize2.prototype.focus=function(container){if(container){this.input.focus()}
this.container.addClass('focus')};Tokenize2.prototype.blur=function(){if(this.isDropdownOpen()){this.trigger('tokenize:dropdown:hide')}
this.container.removeClass('focus');this.resetPending();if(!this.tokensContainer.attr('tabindex')){this.tokensContainer.attr('tabindex',this.options.tabIndex)}};Tokenize2.prototype.keydown=function(e){if(e.type==='keydown'){switch(e.keyCode){case KEYS.BACKSPACE:if(this.input.val().length<1){e.preventDefault();if($('li.token.pending-delete',this.tokensContainer).length>0){this.trigger('tokenize:tokens:remove',[$('li.token.pending-delete',this.tokensContainer).first().attr('data-value')])}else{var $token=$('li.token:last',this.tokensContainer);if($token.length>0){this.trigger('tokenize:tokens:markForDelete',[$token.attr('data-value')]);$token.addClass('pending-delete')}}
...
break;case KEYS.ENTER:this.pressedDelimiter(e);break;case KEYS.ESCAPE:this.resetPending();break;case KEYS.ARROW_UP:e.preventDefault();this.trigger('tokenize:dropdown:up');break;case KEYS.ARROW_DOWN:e.preventDefault();this.trigger('tokenize:dropdown:down');break;case KEYS.CTRL:this.control=!0;break;default:this.resetPending();break}}else{e.preventDefault()}};Tokenize2.prototype.keyup=function(e){if(e.type==='keyup'){switch(e.keyCode){case KEYS.TAB:case KEYS.ENTER:case KEYS.ESCAPE:case KEYS.ARROW_UP:case KEYS.ARROW_DOWN:case KEYS.MAJ:break;case KEYS.CTRL:this.control=!1;break;case KEYS.BACKSPACE:default:if(this.input.val().length>=this.options.searchMinLength&&this.input.val().length>0){this.trigger('tokenize:search',[this.input.val()])}else{this.trigger('tokenize:dropdown:hide')}
break}}else{e.preventDefault()}};Tokenize2.prototype.keypress=function(e){if(e.type==='keypress'){var $delimiter=!1;if(Array.isArray(this.options.delimiter)){if(this.options.delimiter.indexOf(String.fromCharCode(e.which))>=0){$delimiter=!0}}else{if(String.fromCharCode(e.which)===this.options.delimiter){$delimiter=!0}}
...
this.xhr=$.ajax(this.options.dataSource,{data:{search:search},dataType:'json',success:$.proxy(function(data){var $items=[];$.each(data,function(k,v){$items.push(v)});this.trigger('tokenize:dropdown:fill',[$items])},this)})},this),this.options.debounce)};Tokenize2.prototype.dataSourceLocal=function(search){var $searchString=this.transliteration(search);var $items=[];var $pattern=(this.options.searchFromStart?'^':'')+this.escapeRegex($searchString);var $regexp=new RegExp($pattern,'i');var $this=this;$('option',this.element).not(':selected, :disabled').each(function(){if($regexp.test($this.transliteration($(this).html()))){$items.push({value:$(this).attr('value'),text:$(this).html()})}});this.trigger('tokenize:dropdown:fill',[$items])};Tokenize2.prototype.debounce=function(func,threshold){var $args=arguments;var $delayed=$.proxy(function(){func.apply(this,$args);this.debounceTimeout=undefined},this);if(this.debounceTimeout!==undefined){clearTimeout(this.debounceTimeout)}
...
if(el.is('html')){break}}}}
return zindex};Tokenize2.prototype.dropdownHide=function(){if(this.isDropdownOpen()){$(window).off('resize scroll');this.dropdown.remove();this.dropdown=undefined;this.trigger('tokenize:dropdown:hidden')}};Tokenize2.prototype.dropdownClear=function(){if(this.dropdown){this.dropdown.find('.dropdown-menu li').remove()}};Tokenize2.prototype.dropdownFill=function(items){if(items&&items.length>0){this.trigger('tokenize:dropdown:show');this.trigger('tokenize:dropdown:clear');$.each(items,$.proxy(function(k,v){if($('li.dropdown-item',this.dropdown).length<=this.options.dropdownMaxItems){this.trigger('tokenize:dropdown:itemAdd',[v])}},this));if($('li.active',this.dropdown).length<1){$('li:first',this.dropdown).addClass('active')}
...
this.searchContainer.width($width);this.ctx.restore()};Tokenize2.prototype.resetInput=function(){this.input.val('');this.scaleInput()};Tokenize2.prototype.escape=function(string){var $escaping=document.createElement('div');$escaping.innerHTML=string;string=($escaping.textContent||$escaping.innerText||'');return String(string).replace(/["]/g,function(){return'\"'})};Tokenize2.prototype.escapeRegex=function(value){return value.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g,"\\$&")};Tokenize2.prototype.guid=function(){function s4(){return Math.floor((1+Math.random())*0x10000).toString(16).substring(1)}
...
var old=$.fn.tokenize2;$.fn.tokenize2=Plugin;$.fn.tokenize2.Constructor=Tokenize2;$.fn.tokenize2.noConflict=function(){$.fn.tokenize2=old;return this}}))